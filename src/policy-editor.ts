...
import { PACKAGE_REPOSITORY_URL } from "./version.js";
      ...
      await withDataSession(
        ...
        async (session) => {
          for (const id of unresolved) {
            try {
              const { person } = await getPerson(session, id);
              cache.set(id, {
                kind: "person",
                category: "person",
                id,
...
function html(nonce: string, reconnectCommand: string, version: string, repositoryUrl: string): string {
  ...
  const escapedRepositoryUrl = repositoryUrl.replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll('"', "&quot;");
  const reconnectJson = JSON.stringify(reconnectCommand).replaceAll("<", "\\u003c");
  return `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Teams CLI Policy Editor</title>
<style nonce="${nonce}">
:root{color-scheme:light dark;font:14px/1.45 system-ui,sans-serif;--bg:light-dark(#f7f8fb,#161922);--panel:light-dark(#fff,#20252f);--subtle:light-dark(#f0f2f7,#1c202a);--text:light-dark(#172033,#edf0f8);--muted:light-dark(#657087,#a9b1c3);--line:light-dark(#d9dee7,#343947);--accent:light-dark(#4356b8,#6475dc);--danger:light-dark(#a42b25,#ffaaa6);--ok:light-dark(#216a42,#9ae4b8)}*{box-sizing:border-box}body{margin:0;min-height:100vh;display:flex;flex-direction:column;background:var(--bg);color:var(--text)}header{display:flex;align-items:center;gap:12px;padding:12px 16px;background:#252b4a;color:#fff;position:sticky;top:0;z-index:10}header h1{font-size:16px;margin:0 auto 0 0}button,input,select,textarea{font:inherit;font-size:13px}button{border:1px solid var(--line);border-radius:7px;padding:7px 11px;background:var(--panel);color:inherit;cursor:pointer}button:hover{border-color:var(--accent)}button:disabled{opacity:.42;cursor:not-allowed}.primary{background:var(--accent);border-color:transparent;color:#fff}.top-close{background:transparent;color:#fff;border-color:#7780a9}.offline,.banner{padding:10px 14px;margin:10px 20px;border-radius:8px;background:light-dark(#fff4ce,#4a3b17);color:light-dark(#5c4300,#ffe49a)}.banner.error,.offline{background:light-dark(#fee4e2,#4b2020);color:var(--danger)}.hidden{display:none!important}.visually-hidden{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0}.policies{padding:14px 20px;border-bottom:1px solid var(--line);background:var(--subtle)}.policy-head{display:flex;align-items:center;gap:12px;margin-bottom:8px}.policy-head h2{font-size:16px;margin:0}.policy-actions{display:flex;align-items:center;gap:10px;margin-left:auto}.show-all{display:flex;align-items:center;gap:7px;color:var(--muted);font-size:13px}.table-wrap{overflow-x:auto}table{width:100%;min-width:560px;border-collapse:collapse}th,td{padding:8px;text-align:left;border-bottom:1px solid var(--line);font-size:13px}th{color:var(--muted);font-size:12px}.policy-table{min-width:760px;border-top:1px solid var(--line)}.policy-table tbody tr{cursor:pointer}.policy-table tr[aria-current=true]{background:light-dark(#e7eaf6,#292f43);box-shadow:inset 3px 0 var(--accent)}.policy-name{font-weight:600}.badge{display:inline-flex;padding:2px 7px;border-radius:999px;font-size:11px;background:light-dark(#e5e9fb,#353d67);color:light-dark(#354b9a,#c9d1ff)}.badge.active{background:light-dark(#dcefe5,#204733);color:var(--ok)}.badge.error{background:light-dark(#fee4e2,#4b2020);color:var(--danger)}.file{display:flex;align-items:center;gap:7px;white-space:nowrap}.file button{padding:3px 7px;border:0;background:transparent;color:var(--accent)}main{flex:1;padding:0 20px 20px;max-width:1100px;width:100%}.section{padding:17px 0;border-bottom:1px solid var(--line)}.section h3{font-size:16px;margin:0 0 4px}.section-lead{color:var(--muted);margin:0 0 12px}.tabs{display:flex;gap:4px;margin-top:14px;border-bottom:1px solid var(--line)}.tabs button{border-radius:7px 7px 0 0;border-bottom:0}.tabs button[aria-selected=true]{background:var(--accent);color:#fff;border-color:transparent}.search{margin:12px 0}.search-control{position:relative}.search-control input{padding-right:38px}.spinner{position:absolute;right:12px;top:50%;width:16px;height:16px;margin-top:-8px;border:2px solid var(--line);border-top-color:var(--accent);border-radius:50%;animation:spin .7s linear infinite}.search input,input[type=text],textarea,select{width:100%;min-width:0;padding:8px 10px;border:1px solid var(--line);border-radius:7px;background:var(--panel);color:inherit}.suggestions{border:1px solid var(--line);border-radius:8px;margin-top:6px;background:var(--panel)}.suggestion{display:flex;align-items:center;gap:10px;padding:9px 10px;border-top:1px solid var(--line)}.suggestion:first-child{border-top:0}.suggestion>.resource-label{margin-right:auto}.resource-label{display:flex;align-items:center;gap:9px}.resource-label>span:last-child{min-width:0}.resource-icon{display:inline-flex;align-items:center;justify-content:center;width:28px;height:28px;flex:0 0 28px;border-radius:7px;background:light-dark(#e8ebf8,#303750);color:var(--accent)}.resource-icon.chat{background:light-dark(#e5f2ec,#263f35);color:light-dark(#26704b,#8bd5ad)}.resource-icon.channel{background:light-dark(#f7eadf,#433429);color:light-dark(#a05222,#f1b184)}.resource-icon svg{width:18px;height:18px;fill:none;stroke:currentColor;stroke-width:1.6;stroke-linecap:round;stroke-linejoin:round}.resource-icon.chat circle{fill:currentColor;stroke:none}.detail{display:block;margin-top:2px;color:var(--muted);font-size:12px;font-weight:400}.access{min-width:92px}.default-row td{background:var(--subtle);border-top:1px solid var(--line)}.identity-picker{width:100%}.paths{width:100%;min-height:calc(3 * 1.4em + 18px);max-height:calc(6 * 1.4em + 18px);line-height:1.4;resize:vertical}.token{display:flex;align-items:flex-start;gap:9px}.token input{margin-top:4px}.footer{display:flex;align-items:center;gap:9px;padding-top:16px}.footer .dirty{margin-right:auto;color:var(--muted);font-size:13px}.app-footer{display:flex;align-items:center;gap:16px;padding:12px 20px;border-top:1px solid var(--line);background:var(--subtle);color:var(--muted);font-size:12px}.app-footer a{display:inline-flex;align-items:center;gap:6px;color:inherit;text-decoration:none}.app-footer a:hover{color:var(--accent)}.app-footer svg{width:16px;height:16px;fill:currentColor}pre{white-space:pre-wrap;overflow-wrap:anywhere;background:#101526;color:#e8ecf5;padding:12px;border-radius:7px}@keyframes spin{to{transform:rotate(360deg)}}@media(max-width:650px){header,.policy-head,.policy-actions,.footer{align-items:flex-start;flex-wrap:wrap}.policy-actions{margin-left:0}main{padding:0 12px 12px}.app-footer{padding:10px 12px;justify-content:space-between}}
</style></head><body>
<header><h1 id="editorTitle">Policy Editor</h1><button id="closeEditor" class="top-close">Close without saving</button></header>
...
<footer class="app-footer"><span>teams-cli ${escapedVersion}</span><a href="${escapedRepositoryUrl}" target="_blank" rel="noopener noreferrer" aria-label="Open the teams-cli repository on GitHub"><svg viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="M12 .7a11.5 11.5 0 0 0-3.6 22.4c.6.1.8-.2.8-.6v-2.2c-3.3.7-4-1.4-4-1.4-.5-1.4-1.3-1.8-1.3-1.8-1.1-.7.1-.7.1-.7 1.2.1 1.8 1.2 1.8 1.2 1.1 1.8 2.8 1.3 3.5 1 .1-.8.4-1.3.8-1.6-2.7-.3-5.5-1.3-5.5-5.7 0-1.3.5-2.3 1.2-3.1-.1-.3-.5-1.6.1-3.1 0 0 1-.3 3.2 1.2a11 11 0 0 1 5.8 0c2.2-1.5 3.2-1.2 3.2-1.2.6 1.5.2 2.8.1 3.1.8.8 1.2 1.8 1.2 3.1 0 4.4-2.8 5.4-5.5 5.7.4.4.8 1.1.8 2.2v3.3c0 .4.2.7.8.6A11.5 11.5 0 0 0 12 .7Z"/></svg><span>GitHub</span></a></footer>
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      if (request.method === "GET" && (url.pathname === "/" || url.pathname.startsWith("/claim/"))) {
        ...
        const body = html(nonce, restart, options.version, options.repositoryUrl ?? PACKAGE_REPOSITORY_URL);
